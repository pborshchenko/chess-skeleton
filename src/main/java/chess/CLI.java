...
public class CLI {
        ...
        while (true) {
            ...
            if (input == null) {
                ...
                if (input.equals("help")) {
                    ...
                    writeOutput("====> List of possible movements <====");
                    gameState.getPossibleMovements()
                            .forEach(movement -> writeOutput(movement.toString()));

                } else if (input.startsWith("move")) {

                    if (!gameState.move(new Position(input.split(" ")[1]),
                                   new Position(input.split(" ")[2]))) {
                        writeOutput("====> Can't move <====");
                    } else {
                        if (gameState.validateCheck()) {
                            writeOutput("====> Check! <====");
                        }
                        if (gameState.validateMate()) {
                            writeOutput("====> Mate! <====");
                        }
                    }
    ...
    private void printSquares(int rowLabel, StringBuilder builder) {
        ...
        for (char c = Position.MIN_COLUMN; c <= Position.MAX_COLUMN; c++) {
            IPiece piece = gameState.getPieceAt(String.valueOf(c) + rowLabel);
